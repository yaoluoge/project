...
QString DataReport::path_{qApp->applicationDirPath() + "/temp/"};
void DataReport::creatUavsReportHead(QStringList &list, const UavsReportData &reportData)
{
    ...
    // 告警日期+报告时间
    list << "<tr>";
    list << QString("<td>%1</td>").arg(QString("告警时间: %1").arg(reportData.timeAlarm));
...
void DataReport::creatUavsReportBody(QStringList &list, const UavsReportData &reportData, int imageWidth)
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        {
            QString name = path_ + QString("%1-%2.jpg").arg(time).arg(i);
    ...
    for (int i = 0; i < count; i += 2)
    {
        ...
        list << QString("<td align='center'><img src='%1'></td>").arg(listImage.at(i));
        // 需要过滤判断下,很可能是奇数会超过下一个
        if (i < count - 1)
        {
            list << QString("<td align='center'><img src='%1'></td>").arg(listImage.at(i + 1));
        }
        list << "</tr>";

        list << "<tr style='vertical-align:middle;'>";
        list << QString("<td width=\"25%\">%2</td>").arg(QString("目标距离: %1").arg(reportData.dis_list.at(i)));
        // 需要过滤判断下,很可能是奇数会超过下一个
        if (i < count - 1)
        {
            list << QString("<td width=\"25%\">%2</td>").arg(QString("目标距离: %1").arg(reportData.dis_list.at(i + 1)));
        }
        list << "</tr>";

        list << "<tr style='vertical-align:middle;'>";
        list << QString("<td width=\"25%\">%2</td>").arg(QString("帧索引: %1").arg(reportData.frame_id_list.at(i)));
        // 需要过滤判断下,很可能是奇数会超过下一个
        if (i < count - 1)
        {
            list << QString("<td width=\"25%\">%2</td>").arg(QString("帧索引: %1").arg(reportData.frame_id_list.at(i + 1)));
        }
        list << "</tr>";
    }
...
void DataReport::creatPersonReportBody(QStringList &list, const PersonReportData &reportData, const QStringList &columnValue, int brCount)
{
    ...
    // 表格开始
    list << "<table border='0.5' cellspacing='0' cellpadding='5' style='font-size:16px;'>";
