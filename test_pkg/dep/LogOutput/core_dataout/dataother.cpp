...
void DataOther::toPrint(const QPixmap &pixmap, qreal ratio, const QString &fileName)
{
    //保存图片
...
void DataOther::widgetToPrint(QWidget *widget, qreal ratio)
{
#if (QT_VERSION >= QT_VERSION_CHECK(5,0,0))
    ...
    QPixmap pixmap = QPixmap::grabWindow(widget->winId());
#endif
    toPrint(pixmap, ratio);
}

void DataOther::imageToPrint(const QString &imageFile, qreal ratio)
{
    toPrint(QPixmap(imageFile), ratio);
}

void DataOther::chartToPrint(qreal ratio, int width, int height)
{
#ifdef qchart
