...
private:
  void readParam()
  {
    pnh_.param<std::string>("icon_predfix_name", icon_predfix_name, "bubble_marked_frame_");
    ...
    pnh_.param<std::string>("current_task_image_folder", current_task_image_folder, "");
...
public:
  JsonToCsvConverter(QWidget* parent = nullptr) : QWidget(parent)
  {
    ...
    connect(convertButton, &QPushButton::clicked, this, &JsonToCsvConverter::convertJsonToCsv);
...
private slots:
  void convertJsonToCsv()
  {
    QString jsonFilePath =
        QFileDialog::getOpenFileName(this, "Select JSON File", QDir::currentPath(), "JSON Files (*.json)");
    ...
    tableWidget->setHorizontalHeaderLabels(QStringList() << "detection_time"
                                                         << "flatness_tolerance");
    ...
    for (size_t i = 0; i < flatnessArray.size(); ++i)
    {
      ...
      QTableWidgetItem* timeItem = new QTableWidgetItem(QString::fromStdString(flatnessArray[i]["detection_time"]));
      QTableWidgetItem* toleranceItem =
          new QTableWidgetItem(QString::fromStdString(flatnessArray[i]["flatness_tolerance"]));
      tableWidget->setItem(i, 0, timeItem);
      tableWidget->setItem(i, 1, toleranceItem);
    }

    QString csvFilePath = QFileDialog::getSaveFileName(this, "Save as CSV", QDir::currentPath(), "CSV Files (*.csv)");
    ...
    {
      out << tableWidget->item(i, 0)->text() << "," << tableWidget->item(i, 1)->text() << endl;
    }
...
public:
  LogOutWidget(QWidget* parent = nullptr) : QWidget(parent), param_{ flatness_detection::Param::getInstance() }
  {
    ...
    connect(DataPrint::dataPrint, &DataPrint::receiveHtml, [&](const QString& html) { text_edit_->setHtml(html); });
    ...
    connect(btn_out_.get(), &QPushButton::clicked, [&]() {
      ...
      if (!QDir(QString::fromStdString(base_path_ + "/" + folder_ + "/" + current_task_image_folder_)).exists())
      {
    ...
    connect(btn_print_.get(), &QPushButton::clicked, [&]() {
      auto image_path =
          QFileDialog::getExistingDirectory(this, u8"选择日志文件", QString::fromStdString(base_path_ + "/" + folder_));
    ...
    connect(btn_clear_.get(), &QPushButton::clicked, [&]() { text_edit_->clear(); });
  }
...
private:
  ...
  inline void initform()
  {
    ...
    btn_out_tasks_ = QSharedPointer<QPushButton>(new QPushButton(u8"输出需要日志"));
  ...
  inline void dataOutType(const Key& type, const std::string& current_task_image_folder = "")
  {
    DataContent dataContent;
    dataContent.fileName = QString::fromStdString(base_path_ + "/" + folder_pdf_ + "/dataout.pdf");
    ...
    dataContent.html = getAirDetectionReportHtml(reportData, current_task_image_folder);
  ...
  QString getAirDetectionReportHtml(UavsReportData& reportData, const std::string& current_task_image_folder = "")
  {
    QString db_path;
    if (current_task_image_folder == "")
      const_cast<std::string&>(current_task_image_folder) = current_task_image_folder_;
    db_path = QString::fromStdString(base_path_ + "/" + folder_ + "/" + current_task_image_folder);
    QDir dir(db_path);
    ...
    for (const auto& file : file_list)
    {
      ...
      if (match.hasMatch())
      {
        ...
        if (conversion_ok)
        {
          image_name_list.emplace_back(icon_predfix_name_ + std::to_string(number) + ".jpg");
          // ROS_WARN_STREAM(icon_predfix_name_ + std::to_string(number) + ".jpg");
        }
      }
    }
    QStringList images;
    ...
    // reportData.timeAlarm = getAirDetectionTime(QString::fromStdString(current_task_image_folder));
    reportData.dis_list = readJsonInfo(base_path_ + "/" + folder_, current_task_image_folder, Key::DIS);
    reportData.frame_id_list = readJsonInfo(base_path_ + "/" + folder_, current_task_image_folder, Key::FRAME_ID);
    reportData.timeAlarm = readJsonInfo(base_path_ + "/" + folder_, current_task_image_folder, Key::TASK_TIME)[0];
    ROS_ERROR_STREAM(base_path_ + "/" + folder_ + "/" + current_task_image_folder);
    if (reportData.timeAlarm == "nan" | reportData.dis_list[0] == "nan" | reportData.frame_id_list[0] == "nan")
      return "nan";
    // ROS_WARN_STREAM(images.join("").toStdString());
    // 获取打印区域的尺寸用于计算图片的宽度
    // 多减一点是为了有点冗余
    QRectF rect = DataPrint::dataPrint->getPrinter()->pageRect(QPrinter::DevicePixel);
  ...
  inline QString getAirDetectionTime(const QString& input_string)
  {
    ...
    {
      regex = QRegularExpression(QString::fromStdString(image_folder_prefix_ + "_(\\d{4})_(\\d{2}-\\d{2})_(\\d{2}):("
                                                                               "\\d{2})_(\\d{2})"));
    }
    regex = QRegularExpression(QString::fromStdString("(\\d{4})_(\\d{2}-\\d{2})_(\\d{2}):(\\d{2})_(\\d{2})"));
    auto match = regex.match(input_string);
    if (!match.hasMatch())
    {
      QMessageBox::warning(this, u8"警告", u8"任务时间匹配出错,文件夹名称不正确");
      return "nan";
    }
    auto year = match.captured(1), date = match.captured(2), hour = match.captured(3), minute = match.captured(4),
         second = match.captured(5);
    // "yyyy-MM-dd HH:mm:ss"
    auto alarm_time{ year + "-" + date + " " + hour + ":" + minute + ":" + second };
    return alarm_time;
  }

  inline QStringList readJsonInfo(const std::string& json_folder_path, const std::string& json_name, const Key& key)
  {
    ...
    std::ifstream file{ json_folder_path + "/" + json_name + "/" + json_list[0].toStdString() };
    if (!file.is_open())
    {
    ...
    catch (const json::parse_error& e)
    {
      ...
      Q_EMIT sender_errorLog("json解析错误:" + QString::fromStdString(e.what()));
      return QStringList() << "nan";
    }
    QStringList ret;
    switch (key)
  ...
  {
    auto regex{ QRegularExpression(QString::fromStdString("(\\d{4})_(\\d{2}-\\d{2})_(\\d{2}):(\\d{2})_(\\d{2})")) };
  ...
  inline void setFolderPath(const std::string& base_path, const std::string& folder_path)
  {
    ...
    if (!dir.exists())
    {
      ...
      {
        // setLogError(u8"创建icon文件夹失败:" + QString::fromStdString(icon_folder_path));
        Q_EMIT sender_errorLog(u8"创建icon文件夹失败:" + QString::fromStdString(icon_folder_path));
...
public:
  FlatDetectionWidegt(QWidget* parent = nullptr)
    ...
    , detect_count_{ 0 }
    , report_index_{ 0 }
    , is_set_task_folder_{ false }
  {
    readParam();
    sub_height_delta_R_ = nh_.subscribe<flatness_detect::planeToleranceMsg>(
        height_R_topic_.toStdString(), 1,
        [&](const flatness_detect::planeToleranceMsg::ConstPtr& msg) { this->getHeightDeltaR_cb(msg); });
    sub_plane_tolerance_ = nh_.subscribe<flatness_detect::planeToleranceMsg>(
        plane_tolerance_topic_.toStdString(), 1, [this](const flatness_detect::planeToleranceMsg::ConstPtr& msg) {
          ...
          if (state_auto == false)  //手动模式
          {
            ...
            max_plane_tolerance_ = *std::max_element(detect_time_map_.begin(), detect_time_map_.end());
          ...
          else
          {
            if (msg->plane_tolerance >= param_.flat_normal)
            {
              auto thread{ std::thread(
                  [&]() { this->getPlaneTolerance_cb(msg, std::ref(this->max_plane_tolerance_mutex_)); }) };
              if (thread.joinable())
                thread.join();
            }
            else
            ...
            auto time{ QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss.zzz") };
            lbl_stamp_->setText(u8"时间戳: " + time);
            auto location{ "x= " + QString::number(msg->defect_location_x[get_num_], 'f', 2) +
                           " y= " + QString::number(msg->defect_location, 'f', 2) };
            ++get_num_;
            ROS_WARN_STREAM("msg->defect_location_y size: " << msg->defect_location_y.size());
            if (msg->defect_location_y.size() == 0)
              this->setLogError(QString::number(msg->plane_tolerance, 'f', 2), location,
            ...
            {
              std::lock_guard<std::mutex> lock(this->max_plane_tolerance_mutex_);
              lbl_detect_count_->setNum(static_cast<int>(detect_count_));
            }
          }
        });
    sub_cloud_points_ = nh_.subscribe<sensor_msgs::PointCloud2>(
        ...
        [&](const sensor_msgs::PointCloud2::ConstPtr& msg) { this->disPlayCloudPoints_cb(msg); });
    connect(this, &FlatDetectionWidegt::sender_setCurrentTaskFolder, [&]() {
      setCurrentTaskFolder(current_task_image_folder_);
      param_.current_task_image_folder = current_task_image_folder_.toStdString();
    });
    initForm();
    // json_path_ = "./.flatness_tolerance_" + QDateTime::currentDateTime().toString("yyyy-MM-dd
...
private:
  inline void getHeightDeltaR_cb(const flatness_detect::planeToleranceMsgConstPtr& msg)
  {
    ...
    text_edit_->append(u8"话题: " + QString::fromStdString(msg->cam_topic_name));
    text_edit_->append(u8"旋转矩阵:\n" + QString::number(r(0, 0), 'f', 4) + " " + QString::number(r(0, 1), 'f', 4) +
                       " " + QString::number(r(0, 2), 'f', 4) + "\n" + QString::number(r(1, 0), 'f', 4) + " " +
                       QString::number(r(1, 1), 'f', 4) + " " + QString::number(r(1, 2), 'f', 4) + "\n" +
                       QString::number(r(2, 0), 'f', 4) + " " + QString::number(r(2, 1), 'f', 4) + " " +
                       QString::number(r(2, 2), 'f', 4));
  }

  inline void disPlayCloudPoints_cb(const sensor_msgs::PointCloud2::ConstPtr& msg)
  {
    // ROS_WARN_STREAM("****主线程");
  }
  ...
  inline void setFolderPath(const std::string& base_path, const std::string& folder_path)
  {
    ...
    if (!dir.exists())
    {
      ...
      {
        output_widget_->setLogError(QString::fromStdString(u8"创建" + folder_path + u8"文件夹失败:") +
  ...
  inline void setLogError(const QString& log_content, const QString& dis = u8"nan", const QString& idx = u8"nan",
                          const QString& link = u8"nan")
  {
    setLog(report_index_, log_content, false, "", dis, idx, link);
  }

  inline void setLogInfo(const QString& log_content, const QString& dis = "", const QString& idx = "",
                         const QString& link = "")
  {
    setLog(report_index_, log_content, true, "", dis, idx, link);
  }

  inline void setLog(const uint& row, const QString& log_content = "", const bool& is_normal = true,
                     const QString& content_color = "", const QString& dis = "", const QString& idx = "",
                     const QString& link = "")
  {
    if (!is_normal)
  ...
  inline void getPlaneTolerance_cb(const flatness_detect::planeToleranceMsg::ConstPtr& msg, std::mutex& mutex)
  {
    if (!is_set_task_folder_)
    {
      Q_EMIT sender_setCurrentTaskFolder();
      is_set_task_folder_ = true;
      start_report_time_ = QDateTime::currentDateTime().toString("yyyy_MM-dd_hh:mm_ss").toStdString();
    ...
    }
    {
      ...
      detect_time_map_[msg->plane_tolerance] = stamp;
      max_plane_tolerance_ = *std::max_element(detect_time_map_.begin(), detect_time_map_.end());
      auto time{ QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss.zzz").toStdString() };
      ...
      if (msg->defect_location_y.size() == 0)
      {
        ...
        auto defect_location_x{ [&]() -> std::vector<std::string> { return { "nan" }; }() };
        auto defect_location_y{ [&]() -> std::vector<std::string> { return { "nan" }; }() };
        if (!setJsonInfo(base_path + current_task_image_folder_.toStdString(), pair.first, pair.second,
                         defect_location_x, QString::number(msg->defect_location, 'f', 4).toStdString(),
                         defect_location_y, QString::number(msg->measure_num).toStdString()))
          return;
        // nan可能是未受到缺陷值问题导致的
      }
      else
  ...
  inline std::vector<std::string> convertVector(const std::vector<double>& double_vector)
  {
  ...
  inline bool setJsonInfo(const std::string& file_path, const std::string& detection_time,
                          const std::string& flatness_tolerance, const std::vector<std::string>& detection_location_x,
                          const std::string& detection_location, const std::vector<std::string>& detection_location_y,
                          const std::string& measure_num)
  {
    json detection_location_x_json{ { "x", detection_location_x }, { "detect_count", detect_count_ } };
    json detection_location_y_json{ { "y", detection_location_y }, { "detect_count", detect_count_ } };
    ...
    auto json_path{ QString::fromStdString(file_path + "/") + "." + current_task_image_folder_ + ".json" };
    ...
    if (file.open(QIODevice::WriteOnly | QIODevice::Text))
    {
      ...
      file.close();
      chmod(json_path.toStdString().c_str(), S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
      return true;
  ...
  inline void readParam()
  {
    ...
    plane_tolerance_topic_ = QString::fromStdString(param_.plane_tolerance_topic);
    height_R_topic_ = QString::fromStdString(param_.height_R_topic);
    current_task_image_folder_ = QString::fromStdString(param_.current_task_image_folder);
  ...
  {
    auto output_path{ ros::package::getPath("flatness_detect") + "/src/.terminal_info.txt" };
    ...
    if (!file_terminal_.exists())
      ...
      file_terminal_.open(QIODevice::Truncate | QIODevice::WriteOnly | QIODevice::Text);
  ...
  inline void setCurrentTaskFolder(const QString& folder_name = "")
  {
    if (folder_name == "")
    {
      ...
      const_cast<QString&>(folder_name) = current_time.toString("yyyy_MM-dd_hh:mm_ss");
    }
    auto base_path{ path_ + "/" + folder_ };
    if (QDir(QString::fromStdString(base_path) + "/" + folder_name).exists())
    {
      // param_.image_folder_prefix = folder_name.toStdString();
      // auto last_name{QDateTime::currentDateTime().toString("_yyyy_MM-dd_hh:mm_ss") + QString::number(report_index_)};
      auto last_name{ QDateTime::currentDateTime().toString("_yyyy_MM-dd_hh:mm_ss") };
  ...
  void json_to_csv_file(const json& j, const std::string& filename)
  {
    ...
    // Assuming the JSON is an object with a key that contains an array of detections
    const auto& detections = j.at(current_task_image_folder_.toStdString());
    for (const auto& detection : detections)
    {
      file << detection.at("detection_time") << "," << detection.at("flatness_tolerance") << ","
           << detection.at("measure_num") <<","<<detection.at("detection_location_x")["detect_count"]<< "\n";
    }
    file << "检测位置X,检测位置Y,缺陷值,索引,测量次数\n";  // CSV header
    // file << "detection_location_x,detection_location,detection_location_y,detect_count,measure_num\n";  // CSV header

    // std::string detect_count{ detections[current_task_image_folder_.toStdString()]["detect_count"] };
    for (const auto& detection : detections)
    {
      //   file << detection.at("detection_time") << "," << detection.at("flatness_tolerance") << ","
      ...
      for (const auto& x : detection.at("detection_location_x").at("x"))
      {
        file << x << "," << detection.at("detection_location") << ","
             ...
             << detection.at("detection_location_x")["detect_count"] << "," << detection.at("measure_num") << "\n";
        ++index;
      }
    }
    file.close();
  ...
  inline void initForm()
  {
    ...
    connect(btn_log_.get(), &QPushButton::clicked, [&]() {
      ...
      auto json_filename{ path_ + "/" + folder_ + "/" + current_task_image_folder_.toStdString() + "/" + "." +
      ...
      json_to_csv_file(j,
                       path_ + "/" + folder_ + "/" + current_task_image_folder_.toStdString() + "/" + "/检测结果.csv");
    });
    btn_open_robot_ = QSharedPointer<QPushButton>(new QPushButton(u8"机器控制"));
    connect(btn_open_robot_.get(), &QPushButton::clicked, [&]() {
      // 设置启动终端的命令
      const QString terminal_command{ "gnome-terminal" };
      QDir package_path{ QString::fromStdString(ros::package::getPath("flatness_detect")) };
      const auto arm64_exe_path{ package_path.canonicalPath() +
    ...
    rviz_visualization_manager_.reset(new rviz::VisualizationManager(rviz_render_panel_.get()));
    rviz_render_panel_->initialize(rviz_visualization_manager_->getSceneManager(), rviz_visualization_manager_.get());
    ...
    connect(line_edit_.get(), &QLineEdit::textChanged,
            [&](const QString& text) { rviz_visualization_manager_->setFixedFrame(text); });
    setLayout(main_layout);
    //

    grid_display_.reset(rviz_visualization_manager_->createDisplay("rviz/Grid", "Grid", true));
    cloud_points_display_.reset(rviz_visualization_manager_->createDisplay("rviz/PointCloud2", "PointCloud2", true));
    ...
    connect(spin_point_size_.get(), QOverload<int>::of(&QSpinBox::valueChanged),
            [&](const int& value) { cloud_points_display_->subProp("Size (Pixels)")->setValue(value); });
    ...
    // spin_height_delta_ = QSharedPointer<QDoubleSpinBox>(new QDoubleSpinBox());
    // spin_height_delta_->setReadOnly(true);
    ...
    // lbl_plane_tolerance_ = QSharedPointer<QDoubleSpinBox>(new QDoubleSpinBox());
...
private:
  ...
  QSharedPointer<QDoubleSpinBox> spin_height_delta_, spin_rot_x_, spin_rot_y_, spin_rot_z_, spin_rot_w_;
  QSharedPointer<QLabel> lbl_plane_tolerance_, lbl_max_plane_tolerance_, lbl_detect_count_;
...
int main(int argc, char** argv)
{
  ...
  boost::shared_ptr<FlatDetectionWidegt> flat_widget = boost::make_shared<FlatDetectionWidegt>();
  flat_widget->show();
